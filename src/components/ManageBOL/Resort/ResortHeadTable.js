...
export class ResortHeadTable extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dataOptions: [],
    ...
    addAct() {
        var win = window.open("/resortbody/add", '_blank');
        win.focus();
    }
    viewAct(id)
    {
        var win = window.open("/resortbody/view/"+id, '_blank');
        win.focus();
    }
    editAct(id) {
        var win = window.open("/resortbody/edit/"+id, '_blank');
        win.focus();
    }
    deleteAct(id) {
    ...
    render() {
        ...
        return (
            <div>
                ...
                <Button hidden={this.state.dataLoading} style={{ padding: '10px 10px 10px 10px'}}> Создать акт пересорта </Button>
                {
                    this.state.dataLoading ?
                        <p><em>dd</em></p>
                        : <BootstrapTable data={this.state.dataOptions}  striped hover pagination
                                          fluid style={{ height: '100%', overflow : 'auto', padding: '10px 10px 10px 10px'}}
                        >
