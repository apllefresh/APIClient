...
export class InvoiceHeadTable extends Component {
    constructor(props) {
        super(props);
        this.state = {
            dataOptions: [],
    ...
    addAct() {
        var win = window.open("/invoicebody/add", '_blank');
        win.focus();
    }

    viewAct(id) {
        var win = window.open("/invoicebody/view/" + id, '_blank');
        win.focus();
    }

    editAct(id) {
        var win = window.open("/invoicebody/edit/" + id, '_blank');
        win.focus();
    }
            ...
            <div>
                <Modal show={this.state.showModalDeleteHead} onHide={this.handleCloseModalDeleteHead}>
                    <Modal.Header closeButton>
                        <Modal.Title>Удаление накладной</Modal.Title>
                    </Modal.Header>
                    <Modal.Body>
                        <h4>Удалить акт?</h4>
                    </Modal.Body>
                    ...
                    this.state.dataLoading ?
                        ...
                        : <BootstrapTable data={this.state.dataOptions} striped hover fluid>
                            ...
                            <TableHeaderColumn dataField='RequestId'  headerAlign='center' dataAlign='center'
                                               width='100px'># Заявки</TableHeaderColumn>
                            <TableHeaderColumn dataField="button" dataFormat={this.buttonFormatter.bind(this)} width='135' headerAlign='center'
